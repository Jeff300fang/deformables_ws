...
# Optional: make local sam3 package importable
...
class SAM31VideoNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("checkpoint_path", "/home/jeffreyfang/deformables/src/sam3/checkpoints/sam3.1_multiplex.pt")
        ...
        self.declare_parameter("clip_length", 30)
        ...
        self.get_logger().info(f"Buffering {self.clip_length} frames per SAM video session")
    ...
    def process_clip(self):
        clip_dir = Path(tempfile.mkdtemp(prefix="sam31_clip_"))
        session_id = None
        ...
        try:
            ...
            for idx, frame_bgr in enumerate(frames):
                ...
                self.publish_annotated(annotated_bgr, headers[idx])
            ...
            self.get_logger().info(f"Published SAM video results for {len(frames)} frames")
