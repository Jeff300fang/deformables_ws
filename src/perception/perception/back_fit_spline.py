...
class TAPNextDepthSplineNode(Node):
    def __init__(self):
        super().__init__("back_tapnext_depth_spline_node")

        self.declare_parameter("keypoints_topic", "/back_camera/tapnext/keypoints")
        self.declare_parameter("depth_topic", "/back_camera/depth/image_raw")
        self.declare_parameter("camera_info_topic", "/back_camera/color/camera_info")

        self.declare_parameter("points_3d_topic", "/back/tapnext/rope_points_3d")
        self.declare_parameter("spline_3d_topic", "/back/tapnext/rope_spline_3d")
        ...
        self.position_status = self.create_subscription(
            Bool,
            '/change_view',
            self.position_status_callback,
            1
        )
        self.run_back = False
    ...
    def depth_callback(self, depth_msg):
        ...
        try:
            ...
            rope_pose_points = self.sample_centered_by_euclidean_distance(
                spline_3d[::-1],   # keep this if you want right-to-left
                spacing_m=0.1,
                num_segments=9,
            )
